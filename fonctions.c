...
void reinitialiser(){
    ...
    rt_mutex_acquire(&mutexEtat, TM_INFINITE);
    etatCommRobot = 1; /*Connexion perdue*/
    rt_mutex_release(&mutexEtat);

    message = d_new_message(); /*Envoie message au moniteur*/
    message->put_state(message, 1);
    ...
    while (1){
        ...
        if (status == STATUS_OK) {
            ...
            if (compteur > CPT_MAX){ 
                ...
                transmission = robot->get_vbat(robot,&etatBatterie);

                if (transmission != STATUS_OK) {
                	...
                	gerer_cpt(REINITIALISER);
                	if (etatBatterie == BATTERY_OFF){
						  rt_mutex_acquire(&mutexEtat, TM_INFINITE);
						  etatCommRobot = 1;
						  rt_mutex_release(&mutexEtat);
						  robot->stop(robot); //tester que le robot s'est bien arrete
						}
						
		             message = d_new_message(); /*Envoie message au moniteur*/
						 message->put_battery_level(message, etatBatterie);

						 rt_printf("Envoie du niveau de batterie\n");
						 message->print(message, 100);
